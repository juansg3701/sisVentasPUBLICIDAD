	 	...
	 	if ($request) {
				...
				if(date("y",$b)>date("y")){
				 ...
				 if($query4=="Todas las categorías"){
					...
					->join('empresa as em','s.empresa_id_empresa','=','em.id_empresa')
					->join('empresa_categoria as emc','s.empresa_categoria_id','=','emc.id_empresa_categoria')
					->select('s.id_stock_clientes','s.nombre','s.plu','s.ean','sed.nombre_sede as sede_empresa','sed.nombre_sede as sede_cliente','c.nombre as categoria_especial','s.cantidad','sed.id_sede as id_sede_empresa','sed2.id_sede as id_sede_cliente','s.producto_dados_baja','s.fecha_vencimiento', 's.fecha_registro','e.nombre as empleado_id_empleado', 's.imagen as img','s.precio','ct.nombre as categoria_normal','em.nombre as nombre_empresa','emc.nombre as nombre_subempresa')
					->where('s.nombre','LIKE', '%'.$query0.'%')
					->where('s.plu','LIKE', '%'.$query1.'%')
					->where('sed2.nombre_sede','LIKE', '%'.$query2.'%')
					->orderBy('s.id_stock_clientes', 'desc')
				...
				$cargoUsuario=auth()->user()->tipo_cargo_id_cargo;
	 			...
	 			return view('almacen.inventario.stockclientes.index',["categoria"=>$categoria,"productos"=>$productos,"searchText0"=>$query0,"searchText1"=>$query1,"searchText2"=>$query2,"searchText3"=>$query3,"searchText4"=>$query4,"modulos"=>$modulos,"eanP"=>$eanP,"sedesP"=>$sedesP,"proveedoresP"=>$proveedoresP,"usuarios"=>$usuarios,"sedes"=>$sedes]);
	 		}
	 	}
