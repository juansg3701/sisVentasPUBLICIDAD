...
class reportePedidosC2 extends Controller
{
	 	...
	 	if ($request) {
	 		...
	 		$r=RPedidos2::findOrFail($id1);
	 		$fechaR1=$r->fechaActual;
	 		$r2=RPedidos2::findOrFail($id2);
	 		...
	 		$pedidosProveedor2=DB::table('tp_aproveedor as tc')
	 		->join('empleado as e','tc.empleado_id_empleado','=','e.id_empleado')
	 		->join('proveedor as c','tc.proveedor_id_proveedor','=','c.id_proveedor')
	 		->join('tipo_pago as p','tc.tipo_pago_id_tpago','=','p.id_tpago')
	 		->select('tc.id_rproveedor','tc.noproductos','tc.fecha_solicitud','tc.fecha_entrega','tc.pago_inicial','tc.porcentaje_venta','tc.pago_total', 'e.nombre as empleado', 'c.nombre_empresa as proveedor', 'p.nombre as tipo_pago')
	 	...
	 	return view("almacen.reportes.compararGP2.index",["modulos"=>$modulos, "pedidosProveedor"=>$pedidosProveedor, "searchText"=>$query,"id1"=>$id1,"id2"=>$id2,"reportes"=>$reportes,"fechaR1"=>$fechaR1,"fechaR2"=>$fechaR2, "pedidosProveedor2"=>$pedidosProveedor2]);
