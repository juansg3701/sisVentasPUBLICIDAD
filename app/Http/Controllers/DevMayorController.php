...
class DevMayorController extends Controller
{
		...
		$this->middleware('auth');	
	 	...
	 	public function index(Request $request){
	 		if ($request) {
	 			...
	 			->select('s.id_stock','p.nombre','p.plu','p.ean','sed.nombre_sede','pd.nombre_proveedor','s.cantidad','s.disponibilidad','s.sede_id_sede')
				...
				$cargoUsuario=auth()->user()->tipo_cargo_id_cargo;
	 			...
	 			return view('almacen.pedidosDevoluciones.devolucionMayor.index',["productos"=>$productos,"searchText0"=>$query0,"searchText1"=>$query1,"searchText2"=>$query2,"searchText3"=>$query3, "searchText4"=>$query4, "modulos"=>$modulos,"eanP"=>$eanP]);
	 		}
	 	}
	 	...
	 	public function edit($id){
	 		...
	 		return view("almacen.pedidosDevoluciones.devolucionMayor.edit",["sede"=>$sede,"proveedor"=>$proveedor,"producto"=>$producto,"stock"=>ProveedorSede::findOrFail($id), "modulos"=>$modulos]);
	 	}
	 	...
	 	public function update(ProveedorSedeFormRequest $request, $id){
	 		...
	 		$ps->update();
	 		return Redirect::to('almacen/pedidosDevoluciones/devolucionMayor')->with('msj','Devolución exitosa');
	 	}
