...
class HorarioNominaController2 extends Controller
{
	   public function __construct(){
	 	...
	 	public function index(Request $request){
	 		if ($request) {
	 			...
	 			return view('almacen.nomina.lista_horarios.index',["nominas"=>$nominas, "nominas2"=>$nominas2,"usuarios"=>$usuarios, "modulos"=>$modulos,"searchText0"=>$query0,"searchText1"=>$query1,"searchText2"=>$query2,"searchText3"=>$query3, "nombre"=>$nombre, "desde"=>$desde, "hasta"=>$hasta]);
	 		}
	 	}
	 	...
	 	public function create(Request $request){
	 		...
	 		if ($request) {
	 			...
	 			$hasta=trim($request->get('hasta'));
	 			return view('almacen.nomina.lista_horarios.descargas.pdf',["nominas"=>$nominas, "nominas2"=>$nominas2,"usuarios"=>$usuarios, "modulos"=>$modulos,"searchText0"=>$query0,"searchText1"=>$query1,"searchText2"=>$query2,"searchText3"=>$query3, "nombre"=>$nombre, "desde"=>$desde, "hasta"=>$hasta]);
	 		}
	 	...
	 	public function show(Request $request){
	 		if ($request) {
	 			...
	 			return view('almacen.nomina.lista_horarios.aviso',["nominas"=>$nominas, "nominas2"=>$nominas2,"usuarios"=>$usuarios, "modulos"=>$modulos,"searchText0"=>$query0,"searchText1"=>$query1,"searchText2"=>$query2,"searchText3"=>$query3]);
	 		}
	 	}
	 	...
	 	public function store(NominaHorFormRequest $request){
	 		...
	 		$entrada=$nomina->horaentrada=$request->get('horaentrada');
	 		$salida=$nomina->horasalida=$request->get('horasalida');
			...
			if ($jornadas=='Extradominical') {
	 		...
	 		$nomina->save();
			return Redirect::to('almacen/nomina/lista_horarios');
	 	...
	 	public function destroy($id){
	 		...
	 		$nomina->delete();
	 		return Redirect::to('almacen/nomina/modal');
	 	}
